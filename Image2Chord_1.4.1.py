"""Image to Chord 1.4.1 - Convert images to chords.
...
def create_app_window():
    ...
    play_menu.add_command(compound="left",label="Generate chord", command=generate_chord, accelerator="Alt+G")
...
def convert_image():
    ...
    get_notes_number()
    #notes_number=notes_number-1
    indexed_img=img.convert("P").quantize(colors=notes_number,method=PIL.Image.Quantize.MEDIANCUT,kmeans=2,dither=PIL.Image.Dither.FLOYDSTEINBERG)
    ...
    onepix=imgfile.resize((1,1),PIL.Image.Resampling.BICUBIC)
    px = onepix.load()
    onepix_color=px[0,0]
    get_palette()
    generate_chord()
...
def aboutbox():
    ...
    url_title.configure(text="Image to Chord 1.4.1", font=("Arial",15))
    ...
    url_label2.configure(text="https://fonazzastent.com/turn-images-into-chords/")
    url_label2.bind("<Button-1>", lambda e: callback("https://fonazzastent.com/turn-images-into-chords/"))
